...
public class 메서드2 {
	...
	public static void ticket() {//티켓 권종을 묶음
		do {
			try {
	...
	public static void residentNumber() {//주민번호 입력 식을 묶음
	...
	public static void ticketCount() {//티켓 수 입력식을 묶음 
		do {
	...
	public static void ageAndagetype() {//나이와 나이로 분류되는 나이 타입의 수식을 묶음
	...
	public static void ableType() {//나이 별 분류에 일반, 장애, 유공자, 다자녀로 분류되는 우대사항의 수식을 묶음
		do {
	...
	public static void result() {//상위의 모든 수식을 우선 해결한 뒤 우대사항등이 고려된 가격과 카운트를 통해서 최종적인 가격을 도출하는 수식 및 출력값을 묶음
		total = count * price;
	...
	public static void main(String[] args) {
		...
		while (true) {//내부의 내용을 잘 처리하면 다시 이곳으로 돌아옴.
			...
			residentNumber();//권종을 기점으로, 주민번호, 티켓 수, 나이와 나이타입, 우대조건, 그리고 최종가격을 순서대로 수행하게 정렬함.
			ticketCount();
			ageAndagetype();
			ableType();
			result();			
