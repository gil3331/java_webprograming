...
public class AgeOperationClass {
	...
	public int getAgeType(int age, int tryCount) {// 상위의 memberage라는 결과 값을 age로 받고, 하위의 식으로 나이별로 분류하게 함.

		returnAgeType = 0;
		if (age < 3) {// 에버랜드의 규정상 나이가 3세 미만의 경우는 아이로 분류하여, 표값을 받지 아니함.
			returnAgeType = 0;// 나이가 조건에 해당하는 경우를 0으로 규정
			ageTypeName[tryCount] = "유아";
